...
public:
    ...
    bool Join()
    {
        ...
        if (task_failure_count_ != 0) {
            Print("store[" + std::to_string(config_.index) + "] failed tasks: " +
                  std::to_string(task_failure_count_));
            return false;
        }
        return true;
    }
    ...
    bool ExecuteRequests(const std::vector<RequestPlan>& requests)
    {
        ...
        for (std::size_t i = 0; i < requests.size(); ++i) {
            ...
            if (!slot.done.load(std::memory_order_acquire)) {
                ...
                Print("store[" + std::to_string(config_.index) + "] task[" +
                      std::to_string(i) +
            ...
            if (!slot.success) {
                ++failed;
                Print("store[" + std::to_string(config_.index) + "] task[" +
                      std::to_string(i) + "] failed: " + slot.error);
            }
        }
