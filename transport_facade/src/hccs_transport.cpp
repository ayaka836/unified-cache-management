...
Status HccsTransport::exportControlPlane(std::vector<std::byte>& out) const {
    std::ostringstream stream;
    stream << "device_id=" << options_.device_id << '\n'
           << "rank=" << options_.local_rank << '\n'
           << "pid=" << options_.local_pid << '\n';
    for (const auto& name : options_.notify_names) {
        stream << "notify=" << name << '\n';
    }
...
Status HccsTransport::registerMemory(const MemoryRegion& memory,
    ...
    // Peer pid authorization is driven by the remote control blob in connect().

    const std::string ipc_name(ipc_name_buffer);

    out.region = memory;
    out.transport = protocol();
    const auto* begin = reinterpret_cast<const std::byte*>(ipc_name.data());
    out.import_blob.assign(begin, begin + ipc_name.size());
...
Status HccsTransport::importRemoteMemory(const MemoryExport& desc,
                                         MemoryExport& out) {
    const std::string ipc_name(
            reinterpret_cast<const char*>(desc.import_blob.data()),
            desc.import_blob.size());
    const auto key =
        ipc_name.empty()
            ? std::to_string(reinterpret_cast<uint64_t>(desc.region.addr))
            : ipc_name;
    auto iter = remote_import_cache_.find(key);
    ...
    // rtIpcOpenMemory(&mapped_addr, ipc_name.c_str())
    mapped_addr = desc.region.addr;

    out = desc;
    out.region.addr = mapped_addr;
    remote_import_cache_[key] = out;
    return Status::Ok;
}
